...
from taskjournal.commands import write_file, create_daily_notes_file, finalize_daily_notes, create_week_summary, \
    create_retro_file
...
@patch("builtins.open", new_callable=mock_open)
def test_create_week_summary(mock_file, mock_listdir, base_dir, week_summary_template):
    # Given
...
@patch("os.path.exists", return_value=False)
@patch("builtins.open", new_callable=mock_open)
def test_create_retro_file(mock_file, week_folder, retro_template):
    # When
