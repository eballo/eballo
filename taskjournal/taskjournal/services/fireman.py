...
class FiremanService:
    def __init__(self, create_datetime: datetime, debug: bool = False) -> None:
        self.debug = debug
        self.fireman_weeks = set()
        self.load_and_parse(create_datetime)
    ...
    def load_and_parse(self, create_datetime: datetime) -> None:
        try:
            year = create_datetime.year
    ...
    def is_fireman_week(self, date_obj: date) -> bool:
        """Returns True if the date's week is in the fireman list"""
        normalized_input = self._get_monday(date_obj)
        is_fireman_week = normalized_input in self.fireman_weeks
        logger.info(f"Checking if {date_obj} is a fireman week: {is_fireman_week}")
