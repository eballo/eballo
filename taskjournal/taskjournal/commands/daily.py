...
class DailyCommands:
    ...
    async def _generate_and_write_summary(self, daily_notes_file: str, no_summary: bool) -> None:
        if no_summary:
            return
        ...
        if any(line.strip() for line in note.summary):
            logger.debug("Summary already present — skipping AI generation.")
            return
